...
class TCP4_1661(BaseTest):
    ''' Test to verify Delete Data Connectors '''

    def test_delete_data_connectors(self):
        ...
        # STEP 7: Deleted the data connector created
        self.log.append('STEP 7: Deleted the data connector created////////////////')
        pageConnectors.delete_connector(connectors_result_search)

        pageMenu.goto_connectors()

        # STEP 8: Verify if the data connector was deleted
        try:
            # print('STEP 8: Verify if the data connector was deleted')
            self.log.append('STEP 8: Verify if the data connector was deleted////////////////')
            connectors_result_search = pageConnectors.search_connectors(connectors_data['connectors_name'])
            self.assertTrue(connectors_result_search is None)
            # print('Data connector was found')
        except AssertionError as e:
            raise Exception('Error in create data connector function', e)
            # print('Data connector was not deeleted, an error ocurred')

        # print('Delete connector Test completed')

if __name__ == "__main__":
    import __main__
    output = util.run_test(TCP4_1661, data, __main__)